    ...
    render(){
        ...
        return (
            <div>
                <h2>News Story</h2>
                <ul>
                    { !this.props.newsItemLoading ? <div>
                        <NewsItemDetail data={this.props.newsItem} /> 
                        <CommentsPanel comments={this.props.comments} id={this.props.newsItem._id} />
                        </div> : <div>Loading</div>}
                </ul>
            </div>
        )
...
const mapStateToProps = state => {
    return {
        newsItem: state.newsRedu.newsItem,
        comments: state.newsRedu.newsItem.comments,
        newsItemLoading: state.newsRedu.newsItemLoading
    }
}
